...
| 10  | Add Route Protection Proxy               | Next.js Proxy · Supabase                    | `proxy.ts` — intercepts `/desktop/*` requests; reads Supabase session cookie and Guest sessionStorage signal; redirects unauthenticated traffic to `/login` before render                                                                                                  | ❌ Not Started |
| 11  | Validate Phase 1                         | All Phase 1 tooling                         | Full integration sweep: Guest flow, Admin sign-in, session persistence across reload, JWT in Apollo requests, `/desktop` protection, Storybook green, design token coverage audit                                                                                          | ❌ Not Started |
